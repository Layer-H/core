...
const HEALTH_HUB_NFT_NAME = 'Layer-H Profiles';
const HEALTH_HUB_NFT_SYMBOL = 'LHP';
 ...
 *    1. HealthHub Proxy Admin
 *    2. HealthHub Governance
 ...
 */
task('full-deploy-verify', 'deploys the entire Layer-H with explorer verification').setAction(
  async ({}, hre) => {
    ...
    const healthHubImpl = await deployWithVerify(
      new HealthHub__factory(hubLibs, deployer).deploy(followNFTImplAddress, collectNFTImplAddress, {
        nonce: deployerNonce++,
      }),
      [followNFTImplAddress, collectNFTImplAddress],
      'contracts/core/HealthHub.sol:HealthHub'
    );
    ...
    let data = healthHubImpl.interface.encodeFunctionData('initialize', [
      HEALTH_HUB_NFT_NAME,
    ...
    let proxy = await deployWithVerify(
      new TransparentUpgradeableProxy__factory(deployer).deploy(
        healthHubImpl.address,
        proxyAdminAddress,
        data,
        { nonce: deployerNonce++ }
      ),
      [healthHubImpl.address, deployer.address, data],
      'contracts/upgradeability/TransparentUpgradeableProxy.sol:TransparentUpgradeableProxy'
    );

    // Connect the hub proxy to the HealthHub factory and the governance for ease of use.
    const healthHub = HealthHub__factory.connect(proxy.address, governance);

    console.log('\n\t-- Deploying Health Periphery --');
    const healthPeriphery = await deployWithVerify(
      new HealthPeriphery__factory(deployer).deploy(healthHub.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address],
      'contracts/misc/HealthPeriphery.sol:HealthPeriphery'
    );

    // Deploy collect modules
    console.log('\n\t-- Deploying feeCollectModule --');
    const feeCollectModule = await deployWithVerify(
      new FeeCollectModule__factory(deployer).deploy(healthHub.address, moduleGlobals.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address, moduleGlobals.address],
    ...
    const timedFeeCollectModule = await deployWithVerify(
      new TimedFeeCollectModule__factory(deployer).deploy(healthHub.address, moduleGlobals.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address, moduleGlobals.address],
    ...
    const freeCollectModule = await deployWithVerify(
      new FreeCollectModule__factory(deployer).deploy(healthHub.address, { nonce: deployerNonce++ }),
      [healthHub.address],
    ...
    const feeFollowModule = await deployWithVerify(
      new FeeFollowModule__factory(deployer).deploy(healthHub.address, moduleGlobals.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address, moduleGlobals.address],
    ...
    const profileFollowModule = await deployWithVerify(
      new ProfileFollowModule__factory(deployer).deploy(healthHub.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address],
    ...
    const revertFollowModule = await deployWithVerify(
      new RevertFollowModule__factory(deployer).deploy(healthHub.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address],
    ...
    // new ApprovalFollowModule__factory(deployer).deploy(healthHub.address, {
    // nonce: deployerNonce++,
    // }),
    // [healthHub.address],
    ...
    const followerOnlyReferenceModule = await deployWithVerify(
      new FollowerOnlyReferenceModule__factory(deployer).deploy(healthHub.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address],
    ...
    const uiDataProvider = await deployWithVerify(
      new UIDataProvider__factory(deployer).deploy(healthHub.address, {
        nonce: deployerNonce++,
      }),
      [healthHub.address],
    ...
    const profileCreationProxy = await deployWithVerify(
      new ProfileCreationProxy__factory(deployer).deploy(profileCreatorAddress, healthHub.address, {
        nonce: deployerNonce++,
      }),
      [deployer.address, healthHub.address],
    ...
    await waitForTx(
      healthHub.whitelistCollectModule(feeCollectModule.address, true, { nonce: governanceNonce++ })
    );
    await waitForTx(
      healthHub.whitelistCollectModule(limitedFeeCollectModule.address, true, {
        nonce: governanceNonce++,
      })
    );
    await waitForTx(
      healthHub.whitelistCollectModule(timedFeeCollectModule.address, true, {
        nonce: governanceNonce++,
      })
    );
    await waitForTx(
      healthHub.whitelistCollectModule(limitedTimedFeeCollectModule.address, true, {
        nonce: governanceNonce++,
      })
    );
    await waitForTx(
      healthHub.whitelistCollectModule(revertCollectModule.address, true, {
        nonce: governanceNonce++,
      })
    );
    await waitForTx(
      healthHub.whitelistCollectModule(freeCollectModule.address, true, { nonce: governanceNonce++ })
    );

    // Whitelist the follow modules
    console.log('\n\t-- Whitelisting Follow Modules --');
    await waitForTx(
      healthHub.whitelistFollowModule(feeFollowModule.address, true, { nonce: governanceNonce++ })
    );
    await waitForTx(
      healthHub.whitelistFollowModule(profileFollowModule.address, true, { nonce: governanceNonce++ })
    );
    await waitForTx(
      healthHub.whitelistFollowModule(revertFollowModule.address, true, { nonce: governanceNonce++ })
    );
    // --- COMMENTED OUT AS THIS IS NOT A LAUNCH MODULE ---
    // await waitForTx(
    // healthHub.whitelistFollowModule(approvalFollowModule.address, true, {
    // nonce: governanceNonce++,
    // })
    // );
    ...
    await waitForTx(
      healthHub.whitelistReferenceModule(followerOnlyReferenceModule.address, true, {
        nonce: governanceNonce++,
      })
    );
    ...
    await waitForTx(
      healthHub.whitelistProfileCreator(profileCreationProxy.address, true, {
        nonce: governanceNonce++,
      })
    );
    ...
    const addrs = {
      'healthHub proxy': healthHub.address,
      'healthHub impl:': healthHubImpl.address,
      ...
      'health periphery': healthPeriphery.address,
