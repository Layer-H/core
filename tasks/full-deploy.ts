...
task('full-deploy', 'deploys the entire Layer-H').setAction(async ({}, hre) => {
  ...
  const healthHubImpl = await deployContract(
  ...
  const healthPeriphery = await new HealthPeriphery__factory(deployer).deploy(healthHub.address, {
    nonce: deployerNonce++,
  });
  ...
  //   new ApprovalFollowModule__factory(deployer).deploy(healthHub.address, { nonce: deployerNonce++ })
  // );
  ...
  await waitForTx(
    healthHub.whitelistCollectModule(revertCollectModule.address, true, { nonce: governanceNonce++ })
  );
  await waitForTx(
  ...
  // --- COMMENTED OUT AS THIS IS NOT A LAUNCH MODULE ---
  // await waitForTx(
  // healthHub.whitelistFollowModule(approvalFollowModule.address, true, { nonce: governanceNonce++ })
  // );
