  ...
  context('Generic', function () {
    ...
    context('Negatives', function () {
      it('UserTwo should fail to set the profile URI on profile owned by user 1', async function () {
        await expect(
          healthHub.connect(userTwo).setProfileImageURI(FIRST_PROFILE_ID, MOCK_URI)
      ...
      it('UserTwo should fail to set the profile URI on profile owned by user 1', async function () {
        ...
        await expect(
          healthHub.setProfileImageURI(FIRST_PROFILE_ID, profileURITooLong)
      ...
      it('UserTwo should fail to change the follow NFT URI for profile one', async function () {
        await expect(
          healthHub.connect(userTwo).setFollowNFTURI(FIRST_PROFILE_ID, OTHER_MOCK_URI)
        ).to.be.revertedWith(ERRORS.NOT_PROFILE_OWNER_OR_DISPATCHER);
      });
    });
    ...
    context('Scenarios', function () {
      ...
      it('Default image should be used when no imageURI set', async function () {
        await expect(healthHub.setProfileImageURI(FIRST_PROFILE_ID, '')).to.not.be.reverted;
        const tokenUri = await healthHub.tokenURI(FIRST_PROFILE_ID);
        const metadata = await getMetadataFromBase64TokenUri(tokenUri);
        expect(metadata.name).to.eq(`@${MOCK_PROFILE_HANDLE}`);
        expect(metadata.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
      ...
      it('Default image should be used when imageURI contains double-quotes', async function () {
        ...
        await expect(healthHub.setProfileImageURI(FIRST_PROFILE_ID, imageUri)).to.not.be.reverted;
        const tokenUri = await healthHub.tokenURI(FIRST_PROFILE_ID);
        const metadata = await getMetadataFromBase64TokenUri(tokenUri);
        expect(metadata.name).to.eq(`@${MOCK_PROFILE_HANDLE}`);
        expect(metadata.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
      ...
      it('Should return the correct tokenURI after transfer', async function () {
        const tokenUriBeforeTransfer = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataBeforeTransfer.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
        ...
        const tokenUriAfterTransfer = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataAfterTransfer.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
      ...
      it('Should return the correct tokenURI after a follow', async function () {
        const tokenUriBeforeFollow = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataBeforeFollow.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
        ...
        const tokenUriAfterFollow = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataAfterFollow.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
      ...
      it('User should follow profile 1, user should change the follow NFT URI, URI is accurate before and after the change', async function () {
        ...
        await expect(healthHub.setFollowNFTURI(FIRST_PROFILE_ID, OTHER_MOCK_URI)).to.not.be.reverted;
  ...
  context('Meta-tx', function () {
    ...
    context('Scenarios', function () {
      it('TestWallet should set the profile URI with sig', async function () {
        ...
        const tokenUriBefore = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataBefore.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
        ...
        const tokenUriAfter = await healthHub.tokenURI(FIRST_PROFILE_ID);
        ...
        expect(metadataAfter.description).to.eq(`@${MOCK_PROFILE_HANDLE} - Health profile`);
      ...
      it('TestWallet should set the follow NFT URI with sig', async function () {
        ...
        const followNFTURIBefore = await healthHub.getFollowNFTURI(FIRST_PROFILE_ID);
        ...
        const followNFTURIAfter = await healthHub.getFollowNFTURI(FIRST_PROFILE_ID);
