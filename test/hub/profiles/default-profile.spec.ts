  ...
  context('Generic', function () {
    ...
    context('Negatives', function () {
      it('UserTwo should fail to set the default profile as a profile owned by user 1', async function () {
        await expect(
          healthHub.connect(userTwo).setDefaultProfile(FIRST_PROFILE_ID)
    ...
    context('Scenarios', function () {
      it('User should set the default profile', async function () {
        await expect(healthHub.setDefaultProfile(FIRST_PROFILE_ID)).to.not.be.reverted;
        expect(await healthHub.defaultProfile(userAddress)).to.eq(FIRST_PROFILE_ID);
      });
      ...
      it('User should set the default profile and then be able to unset it', async function () {
        ...
        await expect(healthHub.setDefaultProfile(0)).to.not.be.reverted;
        expect(await healthHub.defaultProfile(userAddress)).to.eq(0);
      });
      ...
      it('User should set the default profile and then be able to change it to another', async function () {
        ...
        await expect(healthHub.setDefaultProfile(2)).to.not.be.reverted;
        expect(await healthHub.defaultProfile(userAddress)).to.eq(2);
      });
  ...
  context('Meta-tx', function () {
    ...
    context('Scenarios', function () {
      it('TestWallet should set the default profile with sig', async function () {
        ...
        const defaultProfileBeforeUse = await healthHub.defaultProfile(testWallet.address);
        ...
        const defaultProfileAfter = await healthHub.defaultProfile(testWallet.address);
      ...
      it('TestWallet should set the default profile with sig and then be able to unset it', async function () {
        ...
        const nonce2 = (await healthHub.sigNonces(testWallet.address)).toNumber();
        ...
        const defaultProfileBeforeUse2 = await healthHub.defaultProfile(testWallet.address);
        ...
        const defaultProfileAfter2 = await healthHub.defaultProfile(testWallet.address);
