...
/**
 * Note: We use the `healthHubImpl` contract to test ERC721 specific events.
 *
...
makeSuiteCleanRoom('Events', function () {
  ...
  context('Misc', function () {
    it('Proxy initialization should emit expected events', async function () {
      ...
      matchEvent(receipt, 'Upgraded', [healthHubImpl.address], proxy);
  ...
  context('Hub Governance', function () {
    it('Governance change should emit expected event', async function () {
      receipt = await waitForTx(healthHub.connect(governance).setGovernance(userAddress));
    ...
    it('Emergency admin change should emit expected event', async function () {
      receipt = await waitForTx(healthHub.connect(governance).setEmergencyAdmin(userAddress));
      expect(receipt.logs.length).to.eq(1);
    ...
    it('Protocol state change by governance should emit expected event', async function () {
      receipt = await waitForTx(healthHub.connect(governance).setState(ProtocolState.Paused));
      ...
      receipt = await waitForTx(healthHub.connect(governance).setState(ProtocolState.Unpaused));
    ...
    it('Protocol state change by emergency admin should emit expected events', async function () {
      await waitForTx(healthHub.connect(governance).setEmergencyAdmin(userAddress));

      receipt = await waitForTx(healthHub.connect(user).setState(ProtocolState.PublishingPaused));
      ...
      receipt = await waitForTx(healthHub.connect(user).setState(ProtocolState.Paused));
  ...
  context('Hub Interaction', function () {
    ...
    it('Profile creation should emit the correct events', async function () {
      ...
      expect(receipt.logs.length).to.eq(2);
      matchEvent(receipt, 'Transfer', [ZERO_ADDRESS, userAddress, FIRST_PROFILE_ID], healthHubImpl);
      matchEvent(receipt, 'ProfileCreated', [
        FIRST_PROFILE_ID,
        userAddress,
        userAddress,
    ...
    it('Setting dispatcher should emit correct events', async function () {
      ...
      receipt = await waitForTx(healthHub.setDispatcher(FIRST_PROFILE_ID, userAddress));
    ...
    it('Following should emit correct events', async function () {
      ...
      receipt = await waitForTx(healthHub.connect(userTwo).follow([FIRST_PROFILE_ID], [mockData]));
      const followNFT = await healthHub.getFollowNFT(FIRST_PROFILE_ID);
      ...
      matchEvent(receipt, 'Transfer', [ZERO_ADDRESS, userTwoAddress, 1], healthHubImpl);
    ...
    it('Collecting should emit correct events', async function () {
      ...
      await waitForTx(
        healthHub.connect(governance).whitelistCollectModule(feeCollectModule.address, true)
      );
      await expect(
      ...
      await waitForTx(healthHub.connect(userTwo).follow([FIRST_PROFILE_ID], [[]]));
      ...
      receipt = await waitForTx(healthHub.connect(userTwo).collect(FIRST_PROFILE_ID, 1, collectData));
