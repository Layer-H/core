...
makeSuiteCleanRoom('deployment validation', () => {
  it('Should fail to deploy a HealthHub implementation with zero address follow NFT impl', async function () {
    await expect(
      new HealthHub__factory(hubLibs, deployer).deploy(ZERO_ADDRESS, userAddress)
    ).to.be.revertedWith(ERRORS.INIT_PARAMS_INVALID);
  });

  it('Should fail to deploy a HealthHub implementation with zero address collect NFT impl', async function () {
    await expect(
      new HealthHub__factory(hubLibs, deployer).deploy(userAddress, ZERO_ADDRESS)
    ).to.be.revertedWith(ERRORS.INIT_PARAMS_INVALID);
  });
  ...
  it('Deployer should deploy a HealthHub implementation, a proxy, initialize it, and fail to initialize it again', async function () {
    ...
    await expect(
      HealthHub__factory.connect(proxy.address, user).initialize('name', 'symbol', userAddress)
    ).to.be.revertedWith(ERRORS.INITIALIZED);
  });
