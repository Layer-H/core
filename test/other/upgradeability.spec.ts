...
import {
  MockHealthHubV2BadRevision__factory,
  MockHealthHubV2__factory,
...
makeSuiteCleanRoom('Upgradeability', function () {
  ...
  // The HealthHub contract's last storage variable by default is at the 23nd slot (index 22) and contains the emergency admin
  ...
  it("Should upgrade and set a new variable's value, previous storage is unchanged, new value is accurate", async function () {
    ...
    await expect(
      MockHealthHubV2__factory.connect(proxyHub.address, user).setAdditionalValue(valueToSet)
    ).to.not.be.reverted;
