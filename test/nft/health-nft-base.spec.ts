...
makeSuiteCleanRoom('Health NFT Base Functionality', function () {
  context('generic', function () {
    it('Domain separator fetched from contract should be accurate', async function () {
      const expectedDomainSeparator = keccak256(
        abiCoder.encode(
          ...
          [
            ...
            keccak256(toUtf8Bytes(HEALTH_HUB_NFT_NAME)),
      ...
      expect(await healthHub.getDomainSeparator()).to.eq(expectedDomainSeparator);
    });
  });
  ...
  context('meta-tx', function () {
    ...
    context('Negatives', function () {
      it('TestWallet should fail to permit with zero spender', async function () {
        ...
        await expect(
          healthHub.permit(ZERO_ADDRESS, FIRST_PROFILE_ID, {
            v,
            r,
            s,
      ...
      it('TestWallet should fail to permit with invalid token ID', async function () {
        ...
        await expect(
          healthHub.permit(userAddress, 0, {
            v,
            r,
            s,
            deadline: MAX_UINT256,
      ...
      it('TestWallet should fail to permit with signature deadline mismatch', async function () {
        ...
        await expect(
          healthHub.permit(userAddress, FIRST_PROFILE_ID, {
            v,
            r,
            s,
            deadline: MAX_UINT256,
      ...
      it('TestWallet should fail to permitForAll with zero spender', async function () {
        ...
        await expect(
          healthHub.permitForAll(testWallet.address, ZERO_ADDRESS, true, {
            v,
            r,
            s,
            deadline: MAX_UINT256,
      ...
      it('TestWallet should fail to permitForAll with signature deadline mismatch', async function () {
        ...
        await expect(
          healthHub.permitForAll(testWallet.address, userAddress, true, {
            v,
            r,
            s,
            deadline: MAX_UINT256,
      ...
      it('TestWallet should fail to burnWithSig with invalid token ID', async function () {
        ...
        await expect(healthHub.burnWithSig(0, { v, r, s, deadline: MAX_UINT256 })).to.be.revertedWith(
      ...
      it('TestWallet should fail to burnWithSig with signature deadline mismatch', async function () {
        ...
        await expect(
          healthHub.burnWithSig(FIRST_PROFILE_ID, { v, r, s, deadline: MAX_UINT256 })
        ).to.be.revertedWith(ERRORS.SIGNATURE_INVALID);
      });
      ...
      it('TestWallet should fail to burnWithSig with invalid deadline', async function () {
        ...
        await expect(
          healthHub.burnWithSig(FIRST_PROFILE_ID, { v, r, s, deadline: '0' })
        ).to.be.revertedWith(ERRORS.SIGNATURE_EXPIRED);
      });
    ...
    context('Scenarios', function () {
      it('TestWallet should permit user, user should transfer NFT, send back NFT and fail to transfer it again', async function () {
        ...
        await expect(
          healthHub.transferFrom(testWallet.address, userAddress, FIRST_PROFILE_ID)
        ).to.not.be.reverted;
        await expect(
          healthHub.transferFrom(userAddress, testWallet.address, FIRST_PROFILE_ID)
        ).to.not.be.reverted;
        await expect(
          healthHub.transferFrom(testWallet.address, userAddress, FIRST_PROFILE_ID)
      ...
      it('TestWallet should sign burnWithSig, user should submit and burn NFT', async function () {
        ...
        await expect(
          healthHub.connect(user).burnWithSig(FIRST_PROFILE_ID, { v, r, s, deadline: MAX_UINT256 })
        ).to.not.be.reverted;
      });
    });
  });
