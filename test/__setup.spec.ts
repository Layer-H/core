...
import { HealthHubLibraryAddresses } from '../typechain-types/factories/HealthHub__factory';
...
export const HEALTH_HUB_NFT_NAME = 'Layer-H Profiles';
export const HEALTH_HUB_NFT_SYMBOL = 'LHP';
export const MOCK_PROFILE_HANDLE = 'plant1ghost.eth';
export const HEALTH_PERIPHERY_NAME = 'HealthPeriphery';
...
export let healthHubImpl: HealthHub;
export let healthHub: HealthHub;
export let currency: Currency;
export let abiCoder: AbiCoder;
export let mockModuleData: BytesLike;
export let hubLibs: HealthHubLibraryAddresses;
export let eventsLib: Events;
export let moduleGlobals: ModuleGlobals;
export let helper: Helper;
export let healthPeriphery: HealthPeriphery;
...
before(async function () {
  ...
  healthHubImpl = await new HealthHub__factory(hubLibs, deployer).deploy(
  ...
  // Connect the hub proxy to the HealthHub factory and the user for ease of use.
  healthHub = HealthHub__factory.connect(proxy.address, user);

  // HealthPeriphery
  healthPeriphery = await new HealthPeriphery__factory(deployer).deploy(healthHub.address);
  healthPeriphery = healthPeriphery.connect(user);

  // Currency
  currency = await new Currency__factory(deployer).deploy();

  // Modules
  freeCollectModule = await new FreeCollectModule__factory(deployer).deploy(healthHub.address);
  ...
  profileFollowModule = await new ProfileFollowModule__factory(deployer).deploy(healthHub.address);
  approvalFollowModule = await new ApprovalFollowModule__factory(deployer).deploy(healthHub.address);
  revertFollowModule = await new RevertFollowModule__factory(deployer).deploy(healthHub.address);
  ...
  await expect(healthHub.connect(governance).setState(ProtocolState.Unpaused)).to.not.be.reverted;
  await expect(
    healthHub.connect(governance).whitelistProfileCreator(userAddress, true)
  ).to.not.be.reverted;
  await expect(
    healthHub.connect(governance).whitelistProfileCreator(userTwoAddress, true)
  ).to.not.be.reverted;
  await expect(
    healthHub.connect(governance).whitelistProfileCreator(userThreeAddress, true)
  ).to.not.be.reverted;
  await expect(
    healthHub.connect(governance).whitelistProfileCreator(testWallet.address, true)
  ).to.not.be.reverted;

  expect(healthHub).to.not.be.undefined;
