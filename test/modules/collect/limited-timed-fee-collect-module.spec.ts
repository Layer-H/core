...
makeSuiteCleanRoom('Limited Timed Fee Collect Module', function () {
  ...
  beforeEach(async function () {
    ...
    await expect(
      healthHub.connect(governance).whitelistCollectModule(limitedTimedFeeCollectModule.address, true)
    ).to.not.be.reverted;
    await expect(
  ...
  context('Negatives', function () {
    ...
    context('Collecting', function () {
      ...
      it('Governance should set the treasury fee BPS to zero, userTwo collecting should not emit a transfer event to the treasury', async function () {
        ...
        const tx = healthHub.connect(userTwo).collect(FIRST_PROFILE_ID, 1, data);
      ...
      it('UserTwo should fail to collect without following', async function () {
        ...
        await expect(
          healthHub.connect(userTwo).collect(FIRST_PROFILE_ID, 1, data)
      ...
      it('UserTwo should actuate the original post, fail to collect from their actuate after the collect end timestamp', async function () {
        const secondH_ProfileId = FIRST_PROFILE_ID + 1;
        await expect(
          healthHub.connect(userTwo).createProfile({
            to: userTwoAddress,
  ...
  context('Scenarios', function () {
    ...
    it('User should post with limited timed fee collect module as the collect module and data, fetched prescription data should be accurate', async function () {
      const collectModuleInitData = abiCoder.encode(
        ['uint256', 'uint256', 'address', 'address', 'uint16', 'bool'],
        [
          DEFAULT_COLLECT_LIMIT,
    ...
    it('User should post with limited timed fee collect module as the collect module and data, allowing non-followers to collect, user two collects without following, fee distribution is valid', async function () {
      ...
      await expect(healthHub.connect(userTwo).collect(FIRST_PROFILE_ID, 1, data)).to.not.be.reverted;
    ...
    it('User should post with limited timed fee collect module as the collect module and data, user two actuates, follows, then collects from their actuate and pays fee, fee distribution is valid', async function () {
      const secondH_ProfileId = FIRST_PROFILE_ID + 1;
      const collectModuleInitData = abiCoder.encode(
        ['uint256', 'uint256', 'address', 'address', 'uint16', 'bool'],
        [
    ...
    it('User should post with limited timed fee collect module as the collect module and data, with no referral fee, user two actuates, follows, then collects from their actuate and pays fee, fee distribution is valid', async function () {
      const secondH_ProfileId = FIRST_PROFILE_ID + 1;
      const collectModuleInitData = abiCoder.encode(
        ['uint256', 'uint256', 'address', 'address', 'uint16', 'bool'],
    ...
    it('User should post with limited timed fee collect module as the collect module and data, user two actuates, follows, then collects once from the original, twice from the actuate, and fails to collect a third time from either the actuate or the original', async function () {
      const secondH_ProfileId = FIRST_PROFILE_ID + 1;
      const collectModuleInitData = abiCoder.encode(
        ['uint256', 'uint256', 'address', 'address', 'uint16', 'bool'],
        [
      ...
      await expect(healthHub.connect(userTwo).collect(FIRST_PROFILE_ID, 1, data)).to.be.revertedWith(
