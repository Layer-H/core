...
makeSuiteCleanRoom('Revert Follow Module', function () {
  beforeEach(async function () {
    ...
    await expect(
      healthHub.connect(governance).whitelistFollowModule(revertFollowModule.address, true)
    ).to.not.be.reverted;
  });
  ...
  context('Negatives', function () {
    ...
    context('Processing follow', function () {
      it('UserTwo should fail to process follow', async function () {
        await healthHub.setFollowModule(FIRST_PROFILE_ID, revertFollowModule.address, []);
        expect(await healthHub.getFollowModule(FIRST_PROFILE_ID)).to.be.equal(
          revertFollowModule.address
        );
        await expect(healthHub.connect(userTwo).follow([FIRST_PROFILE_ID], [[]])).to.be.revertedWith(
