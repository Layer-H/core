...
contract HealthPeriphery {
    ...
    bytes32 internal constant TOGGLE_FOLLOW_WITH_SIG_TYPEHASH =
        keccak256(
            'ToggleFollowWithSig(uint256[] H_profileIds,bool[] enables,uint256 nonce,uint256 deadline)'
        );
    bytes32 internal constant SET_PROFILE_METADATA_WITH_SIG_TYPEHASH =
        keccak256(
            'SetProfileMetadataURIWithSig(uint256 H_profileId,string metadata,uint256 nonce,uint256 deadline)'
        );
     ...
     */
    function setProfileMetadataURI(uint256 H_profileId, string calldata metadata) external {
     ...
     */
    function toggleFollow(uint256[] calldata H_profileIds, bool[] calldata enables) external {
     ...
     */
    function getProfileMetadataURI(uint256 H_profileId) external view returns (string memory) {
        return _metadataByProfile[H_profileId];
    }

    function _setProfileMetadataURI(uint256 H_profileId, string calldata metadata) internal {
    ...
    function _toggleFollow(
        ...
        uint256 H_profileIdsLength = H_profileIds.length;
        for (uint256 i = 0; i < H_profileIdsLength; ) {
